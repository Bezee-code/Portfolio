...
const srcBrainPath = 'C:\\Users\\Souvik Ghosh\\.gemini\\antigravity\\brain\\6d8b40d9-0c7f-4eee-a888-15f179dbd14e\\hospital_onboarding_portfolio_prototype.html';
const appDir = __dirname;
const destHospitalHtml = path.join(appDir, 'hospital.html');
const destHospitalProto = path.join(appDir, 'hospital_onboarding_portfolio_prototype.html');

// 1. Copy prototype file if needed
let hospitalContent = '';
if (fs.existsSync(srcBrainPath)) {
  hospitalContent = fs.readFileSync(srcBrainPath, 'utf8');
} else if (fs.existsSync(destHospitalHtml)) {
  hospitalContent = fs.readFileSync(destHospitalHtml, 'utf8');
}

// Add Back to Portfolio button to hospital.html if missing
...
if (!hospitalContent.includes('Back to Portfolio Homepage')) {
  hospitalContent = hospitalContent.replace(
    '<header class="portfolio-header">\n        <div class="header-title-block">\n            <h1>',
    backBtnHeader
  );
}

fs.writeFileSync(destHospitalHtml, hospitalContent, 'utf8');
fs.writeFileSync(destHospitalProto, hospitalContent, 'utf8');
console.log('Saved hospital.html & hospital_onboarding_portfolio_prototype.html with Back button!');

// 2. Inject Project 5 into index.html directly below Snapmint
...
const hospitalCaseCard = `
        ...
        <div class="pm-case-card clickable-card" onclick="if(!event.target.closest('a')) window.location.href='hospital.html';" style="cursor:pointer; border-color:var(--border-highlight); margin-top:3.5rem;">
...
if (!indexContent.includes('hospital.html')) {
  ...
  if (snapmintPos !== -1) {
    ...
    if (cardEndPos !== -1) {
      ...
      console.log('Injected Project 5 (Healthcare Agentic AI Suite) directly below Snapmint in index.html!');
    }
  }
}

// 3. Update js/data.js
const dataFile = path.join(appDir, 'js', 'data.js');
...
if (!dataContent.includes('healthcare-agentic-ai-suite')) {
  ...
  dataContent = dataContent.replace('PM Projects (4)', 'PM Projects (5)');
...
console.log('Complete hospital onboarding integration finished!');
